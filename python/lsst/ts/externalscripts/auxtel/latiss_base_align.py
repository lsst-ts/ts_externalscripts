...
class LatissBaseAlign(salobj.BaseScript, metaclass=abc.ABCMeta):
    ...
    **Details**
    ...
    def __init__(self, index=1, remotes=True, descr=""):
        ...
        # corrections. See https://sitcomtn-072.lsst.io/ for information about
    ...
    def calculate_results(self) -> LatissAlignResults:
        ...
        zern_str = (len(self.zern) * "{:0.1f}, ").format(*self.zern)
        rot_zern_str = (len(rot_zern) * "{:0.1f}, ").format(*rot_zern)
        hexapod_str = (len(hexapod_offset) * "{:0.3f}, ").format(*hexapod_offset)
