    ...
    * Run a LOVE uptime test by generating several client connections
    ...
    async def run(self):
        ...
        for name_index in self.remotes:
            event_streams[name_index] = ["heartbeat"]
            telemetry_streams[name_index] = []
        ...
        while True:
            ...
            if current_uptime != 0 and current_uptime < 0.999:
                break

            await asyncio.sleep(self.loop_time_send_commands)
            random_csc = random.choice(list(self.remotes.keys()))
            name, salindex = salobj.name_to_name_index(random_csc)
            try:
                await self.client.send_SAL_command(name, salindex, "cmd_setLogLevel", {"level": 10})
